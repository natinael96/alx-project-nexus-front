    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {savedJobs.length === 0 ? (
        <div className="bg-white rounded-xl border border-neutral-200 p-12 text-center">
          ...
          <p className="text-sm text-neutral-400 mb-4">Browse jobs to save interesting ones.</p>
          <Link
            to="/"
            ...
            .map((savedJob) => (
              <div key={savedJob.id} className="bg-white rounded-xl border border-neutral-200 p-6 hover:border-neutral-300 hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-3">
                  <Link to={`/jobs/${savedJob.job_detail.id}`} className="flex-1">
                    <h3 className="text-lg font-medium text-neutral-900 hover:text-neutral-700 mb-1">
                      {savedJob.job_detail.title}
                    </h3>
                    <p className="text-sm text-neutral-500">{savedJob.job_detail.employer?.username}</p>
                  </Link>
                  <div onClick={(e) => e.preventDefault()} className="relative z-10">
                    <SaveJobButton
                      jobId={savedJob.job_detail.id}
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3 text-xs text-neutral-500">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {savedJob.job_detail.job_type?.replace('-', ' ')}
                  </span>
                </div>
